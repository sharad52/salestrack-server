...
async def create_user(request: Request, payload: schema.UserBaseSchema):
    try:
...
async def login(request: Request, payload: schema.UserBaseSchema):
    try:
        db = request.app.state.db
...
@router.post('/logout')
async def logout(request: Request, dependencies=Depends(jwt_utils.jwt_bearer)):
    try:
        db = request.app.state.db
...
def change_password(request: Request, payload: schema.ChangePaaswordBaseSchema):
    try:
        db = request.app.state.db
        user = db.query(User).filter(User.email == payload.email).first()
        if not user:
